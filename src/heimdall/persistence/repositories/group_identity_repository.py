...
from heimdall.persistence.dao import IsxGroupIdentity


class GroupIdentityRepository(AbstractRepository):
    ...
    def query(self, match_pairs: dict) -> list:
        try:
            results = []
            query_result = self.db.session.query(IsxGroupIdentity) \
                .all()
    ...
    def get(self, entity_id) -> dict:
        try:
            query_result = self.db.session.query(IsxGroupIdentity) \
                .filter(IsxGroupIdentity.application_id == str(entity_id)) \
                .all()
            for identity in query_result:
